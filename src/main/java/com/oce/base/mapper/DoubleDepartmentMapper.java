...
public interface DoubleDepartmentMapper {
    ...
    List<Integer> selectChileidByid(@Param("id") Long id);
    ...
    List<TDepartment> selectChileByid(@Param("id") Long id);

    //查询部门下的所有部门id
    @Select("select chile_department_id from t_department_department where father_department_id=#{id}")
    List<Integer> selectAllDepartmentidByid(@Param("id") Long id);
